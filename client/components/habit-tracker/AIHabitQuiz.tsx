...
export default function AIHabitQuiz({
  ...
  const quizQuestions = useMemo(() => [
    ...
    {
      id: "level" as const,
      title: t.quiz.levelTitle,
      subtitle: t.quiz.levelDesc,
      options: [
        t.options.beginner,
        t.options.intermediate,
        t.options.advanced,
      ],
    },
  ], [t]);
  ...
  const handleSelect = (value: string) => {
    ...
    const timeMap: Record<string, string> = {
      ...
      [t.options.thirtyPlusMin]: "30+ min",
    };
    ...
    const levelMap: Record<string, string> = {
      [t.options.beginner]: "beginner",
      [t.options.intermediate]: "intermediate",
      [t.options.advanced]: "advanced",
    };
    ...
    } else if (activeQuestion.id === "level") {
      normalizedValue = levelMap[value] || value.toLowerCase();
    } else {
  ...
  const canContinue = useMemo(() => {
    ...
    if (activeQuestion.id === "level") return Boolean(answers.level);
    return false;
    ...
    <section className="rounded-[32px] border border-border/70 bg-card/85 p-5 shadow-[0_28px_80px_rgba(77,73,122,0.14)] backdrop-blur-xl sm:p-7">
      <div className="flex items-start justify-between gap-4">
          ...
          {results.length === 0 && (
            <div className="mt-4 rounded-[16px] border border-primary/20 bg-primary/5 p-3">
              <p className="text-xs text-muted-foreground">
                <span className="font-semibold text-primary">{t.quiz.smartTip}</span> {t.quiz.howItWorks}
              </p>
            </div>
          )}
        </div>
        ...
        {results.length === 0 ? (
          <div className="hidden rounded-full bg-secondary px-3 py-1 text-xs font-medium text-secondary-foreground sm:block">
            {t.quiz.question} {step + 1} {t.quiz.of} {totalSteps}
          </div>
        ) : null}
      </div>
      ...
      {results.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {isMustHave && (
                          <span className="text-xs font-semibold text-primary">{t.quiz.essential}</span>
                        )}
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-1">
                        {habit.category === "Spirituality" && (
                          <>
                            ...
                            <span className="inline-flex items-center rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] font-medium text-indigo-700">🧘 {t.options.calm}</span>
                          </>
                        )}
                        {habit.category === "Health" && (
                          <>
                            ...
                            <span className="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-medium text-blue-700">⚡ {t.options.physical}</span>
                          </>
                        )}
                        {habit.category === "Learning" && (
                          <>
                            ...
                            <span className="inline-flex items-center rounded-full bg-orange-100 px-2 py-0.5 text-[10px] font-medium text-orange-700">📚 {t.options.discipline}</span>
                          </>
                        )}
                        ...
                        {habit.category === "Financial" && (
                          <>
                            ...
                            <span className="inline-flex items-center rounded-full bg-teal-100 px-2 py-0.5 text-[10px] font-medium text-teal-700">📈 {t.options.productivity}</span>
                          </>
                        )}
                        {habit.category === "Social" && (
                          <>
                            ...
                            <span className="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-medium text-red-700">❤️ {t.options.health}</span>
                          </>
                        )}
                        {habit.category === "Creative" && (
                          <>
                            ...
                            <span className="inline-flex items-center rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-medium text-violet-700">✍️ {t.options.learning}</span>
                          </>
                        )}
                      </div>
                    </div>
        ...
        <div className="mt-8">
            ...
            <div className="flex items-center justify-between gap-2 mb-4">
              {Array.from({ length: totalSteps }).map((_, idx) => (
                ...
                >
                  {/* Step circle */}
                  <motion.div
                    className={`h-12 rounded-full flex items-center justify-center font-semibold text-sm transition-all ${
                      idx < step
                  ...
                  {idx < totalSteps - 1 && (
                    <motion.div
                      className={`absolute top-6 left-1/2 w-full h-0.5 transition-all ${
                        idx < step ? "bg-primary" : "bg-secondary"
                      }`}
                      style={{ marginLeft: "calc(25% + 1rem)" }}
                      initial={{ scaleX: 0 }}
          ...
          {/* Question Section */}
          <motion.div
            className="rounded-[28px] bg-secondary/45 p-5 sm:p-7"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.4 }}
          >
            <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.22em] text-primary/80">
              {t.quiz.question} {step + 1} of {totalSteps}
            </p>
            <motion.h3
              className="mt-3 text-2xl sm:text-3xl font-semibold text-foreground"
            ...
            <div className="mt-6 flex gap-3">
              ...
              >
                {step === totalSteps - 1 ? t.quiz.seeSuggestions : t.quiz.continue}
              </button>
            </div>
          </motion.div>
        </div>
