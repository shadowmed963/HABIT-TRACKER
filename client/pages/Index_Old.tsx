...
import StatsPanel from "@/components/habit-tracker/StatsPanel";
...
import OnboardingWelcome from "@/components/habit-tracker/OnboardingWelcome";
...
import DashboardHeader from "@/components/habit-tracker/DashboardHeader";
...
export default function Index() {
  ...
  const completedCount = useMemo(
    () => (habits ? habits.reduce((total, habit) => total + habit.completions.filter(Boolean).length, 0) : 0),
    [habits],
  );
  ...
  const handleAuthSubmit = async (
    ...
    if (isSupabaseConfigured()) {
      ...
      if (mode === "login") {
        ...
        if (!created) return { ok: false, error: "Failed to create local profile" };
    ...
    <>
      {showLanding && !activeUser ? (
        ...
        <div className="space-y-3 sm:space-y-5">
          <DashboardHeader userName={activeUser?.name} habits={habits} onLogout={logout} />

          {/* Daily AI Message */}
          {onboardingComplete && habits.length > 0 && (
            <DailyAIMessage habits={habits} userName={activeUser?.name} />
          )}

          {/* Key Statistics */}
          {onboardingComplete && habits.length > 0 && (
            <section className="rounded-[28px] border border-border bg-card/50 p-4 backdrop-blur sm:p-6">
              <h3 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground mb-4 sm:mb-5">
                Your Progress
              </h3>
              <StatsPanel habits={habits} createdAt={activeUser?.created_at} />
            </section>
          )}
